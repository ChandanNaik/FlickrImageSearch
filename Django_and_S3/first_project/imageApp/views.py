...
from imageApp.flickrToS3 import performDumpFunction1
...
def loading(request):
	return render(request, 'imageApp/loading.html')
...
def updateUserTags(request):
	if request.method == 'POST':
		if 'searchText' in request.POST:
			...
			userlistTag = searchText.split()
			userTagsCombined = cache.get('prettySearchTags')
			userTags={}
			for i in userlistTag:
				userTags[i]=1
			userTagsCombined["flickrTags"]=userTags
